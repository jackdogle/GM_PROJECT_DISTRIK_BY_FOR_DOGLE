...
public OnPlayerConnect(playerid)
{
    ...
    if((vdLocalStream[playerid] = SvCreateDLStreamAtPlayer(15.0, SV_INFINITY, playerid, 0xffff0000, "")))
    {
        SendClientMessage(playerid, -1, "[i] Selalu ingat bahwa server ini menggunakan sistem voice only, dilarang keras RP Bisu/Tuli.");
        SendClientMessage(playerid, -1, "[i] Selama di dalam server ingatlah aturan pokok District  - > {FFFF00}Respect & Good Attitude.");
...
public OnFilterScriptInit()
{
    ...
    print("-------------------- [District  Voice Chat System] --------------------\
    \n===>> Berhasil dimuat dengan baik!\
    \n===>> by: idann (Alzhidan Putra Syah)\
    \n===>> dipersembahkan untuk District \
    \n-----------------------------------------------------------------");
    return 1;
}
